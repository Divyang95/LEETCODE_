...
export const executeCode = async (req,res)=>{
    try {
       ...
       //4.poll judge0 for results of all submitted cases 
       ...
       const detailedResults = results.map((result,i)=>{
        const stdout = result.stdout?.trim() //this is from result which is provided by judge0
        const expected_output = expected_outputs[i]?.trim(); //which is not in result which comes from frontend
        ...
        // console.log(`Input ${stdin[i]}`);//it is not from judge0 it is coming from frontend 
        // console.log(`Expected Output for the test case ${expected_output}`);
        // console.log(`Actual output from judge0 ${stdout}`)
       ...
       if(allPassed){
        //upsert means if exist then update it and if not exist then create 
       ...
       res.status(200).json({
        ...
        console.error("Error in executing the code:", error.message);
