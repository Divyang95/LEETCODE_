  ...
  return (
    <div className='w-full max-w-6xl mx-auto mt-10'>
        ...
        <div className='flex flex-wrap justify-between items-center mb-6 gap-4'>
            <input type="text" placeholder='Search By Title'
             ...
             <select className="select select-bordered bg-base-200" value={difficulty}
              ...
              <select className='select select-bordered bg-base-200' value={selectedTag}
            ...
            <table className='table table-zebra table-lg bg-base-200 text-base-content'>
                ...
                <tbody>
                    {paginatedProblems.length > 0 ? (
                                    ...
                                    <td>
                                        <div className='flex flex-wrap gap-1'>
                                            {(problem.tags || []).map((tag, idx) => (
                                                <span key={idx} className='badge badge-outline text-xs font-bold'>{tag}</span>
                                            ))}
                                        </div>
                                    </td>
                                    <td>
                                        <span className={`badge font-semibold text-xs text-white ${
                                        ...
                                        <div className='flex flex-col md:flex-row gap-2 items-start md:items-center' >
                                            {
                                                authUser?.role === "ADMIN" && (
                                                    <div className='flex gap-2'>
                                                        <button onClick={()=> (handleDelete(problem.id))} className='btn btn-sm btn-error'>
                                                            {
                        ...
                        <td colSpan={5} className='text-center py-6 text-gray-500'>No Problem Found</td>
                    </tr>) }
                </tbody>
            </table>
        </div>
        ...
        <div className='flex justify-center mt-6 gap-2'>
            ...
            <button className='btn btn-sm' disabled={currentPage === totalPages} onClick={()=> setCurrentPage((prev) => prev+1)}>
                next 
            </button>
        </div>
              <CreatePlaylistModal
